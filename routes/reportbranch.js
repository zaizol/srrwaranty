...
router.get('/', isLoggedIn, function(req, res) {
    res.render('reportbranch.ejs', { user: req.user,moment:moment });
});
...
router.get('/load', isLoggedIn, function(req, res) {
    ...
    Branch.find().exec(function(err, branchs) {
        ...
        //console.log(tonext_date);
            Customer.find({ 'RegisterInfo.dateofRegister':{ $gte: next_date,$lte:  moment(tonext_date).endOf('day')}}).exec(function(err, customers) {
                var reports = [];
                 var qty=0;
