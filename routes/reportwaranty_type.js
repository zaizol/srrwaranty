...
router.get('/', isLoggedIn, function(req, res) {
    res.render('reportwaranty_type.ejs', { user: req.user,moment:moment });
});
...
router.get('/load', isLoggedIn, function(req, res) {
    ...
    Product.find().lean().distinct('type').exec(function(err, Products) {
                ...
                var reports = [];
                 ...
                 for(var i in Products){
                    ...
                    for (var j in joblists) {
                        joblist = joblists[j];
                            if (Product==joblist.productID.type)
                            {
                                qty=qty+1;
                            }
                        
                    }
                    reports.push({ type: Product,qty:qty });
                 }
